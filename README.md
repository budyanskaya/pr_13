...
![image](https://github.com/user-attachments/assets/b879c92b-c53b-402b-9a94-3f140af7228f)

Был проведен анализ плана выполнения запроса SELECT * FROM table1 t1 JOIN table2 t2 ON t1.id1 = t2.id1;. Локальный анализ показал нам, что основным узлом является Hash Join, который занимает около 223.425 мс, а аналогичный анализ на сайте подтвердил то, что ХЭШ-СОЕДИНЕНИЕ является самым дорогим узлом, хотя общее время выполнения составило 236.55 мс, эта разница может быть связана с различиями в конфигурации сервера.
